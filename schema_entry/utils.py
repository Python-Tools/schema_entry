...
def _argparse_base_handdler(_type: Any, key: str, schema: Dict[str, Any], parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    kwargs: Dict[str, Any] = {}
    kwargs.update({
        "type": _type
    })
...
def _argparse_number_handdler(key: str, schema: Dict[str, Any], parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    return _argparse_base_handdler(float, key, schema, parser)


def _argparse_string_handdler(key: str, schema: Dict[str, Any], parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    return _argparse_base_handdler(str, key, schema, parser)


def _argparse_integer_handdler(key: str, schema: Dict[str, Any], parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    return _argparse_base_handdler(int, key, schema, parser)
...
def parse_schema_as_cmd(key: str, schema: Dict[str, Any], parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """根据字段的模式解析命令行行为
    ...
    key = key.replace("_", "-")
    if _type == "number":
        return _argparse_number_handdler(key, schema, parser)
    elif _type == "string":
        return _argparse_string_handdler(key, schema, parser)
    elif _type == "integer":
        return _argparse_integer_handdler(key, schema, parser)
    elif _type == "boolean":
